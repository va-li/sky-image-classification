...
preprocess = transforms.Compose(
    [
        transforms.ToTensor(),
        transforms.Resize(224), # mobilenet_v3_large expects 224x224 images, because it was trained on ImageNet dataset which has 224x224 images
        transforms.CenterCrop(224), # resize only makes the smaller side fit 224, so we need to crop to the center
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), # normalize to ImageNet mean and std
    ]
)

# set the device to be always CPU, because we will run the model on the raspberry pi
device = 'cpu'
...
# show the classifier part of the model
print(net.classifier)

while True:
    file_path_str = input('Image file: ')
    if len(file_path_str.strip()) == 0:
        break 
    ...
    image_preprocessed = input_tensor.numpy().copy().transpose((1, 2, 0))
    ...
    # Print top 10 classes predicted by the model
